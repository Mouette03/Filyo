...
import { uploadRequestRoutes } from './routes/uploadRequests.js'
import { adminRoutes } from './routes/admin.js'
import { authRoutes } from './routes/auth.js'
import { userRoutes } from './routes/users.js'
import { settingsRoutes } from './routes/settings.js'
import { runScheduledCleanup, cleanupExpiredTusUploads } from './lib/cleanup.js'
import { UPLOAD_DIR } from './lib/config.js'

const appVersion: string = (() => {
  try {
    return (JSON.parse(readFileSync(path.join(import.meta.dirname, '../package.json'), 'utf-8')) as { version?: string }).version ?? 'unknown'
  } catch {
